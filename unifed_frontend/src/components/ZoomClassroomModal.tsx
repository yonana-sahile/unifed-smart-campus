...
import type { User, ZoomClassSession, ZoomChatMessage, ZoomAttendee } from "../types";
...
interface ZoomClassroomModalProps {
  isOpen: boolean;
  onClose: () => void;
  session: ZoomClassSession;
  currentUser: User;
  onSessionUpdated?: (updated: ZoomClassSession) => void;
}

export function ZoomClassroomModal({
  isOpen, onClose, session: initialSession, currentUser, onSessionUpdated,
}: ZoomClassroomModalProps) {
  const [session, setSession] = useState<ZoomClassSession>(initialSession);
  const isInstructor = currentUser.role === "INSTRUCTOR" || currentUser.role === "DEPARTMENT_HEAD";

  const [isMuted, setIsMuted] = useState(isInstructor ? false : true);
  const [isVideoOn, setIsVideoOn] = useState(true);
  const [isScreenSharing, setIsScreenSharing] = useState(isInstructor);
  const [isRecording, setIsRecording] = useState(session.status === "LIVE");
  const [isHandRaised, setIsHandRaised] = useState(false);

  const [viewMode, setViewMode] = useState<"speaker" | "gallery" | "whiteboard">("speaker");
  ...
  useEffect(() => {
    ...
    const currentAttendees = initialSession.activeAttendees || [];
    const exists = currentAttendees.some((a) => String(a.id) === String(currentUser.id));
    if (!exists) {
      const newAtt: ZoomAttendee = {
        id: String(currentUser.id),
        name: currentUser.fullName + (isInstructor ? " (Instructor)" : ""),
        avatarUrl: (currentUser as any).avatarUrl,
        role: isInstructor ? "INSTRUCTOR" : "STUDENT",
        joinedAt: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
        isMuted: isInstructor ? false : true,
        isVideoOn: true,
        isHandRaised: false
      };
      const updated = { ...initialSession, activeAttendees: [...currentAttendees, newAtt] };
      setSession(updated);
      CampusDatabase.updateZoomSession(String(initialSession.id), { activeAttendees: updated.activeAttendees })
        .then((res) => { if (onSessionUpdated) onSessionUpdated(res); })
        .catch((err) => console.error("Failed to register attendee:", err));
    }
  }, [initialSession.id, currentUser.id]);

  useEffect(() => {
    if (activeDrawer === "chat") {
      chatBottomRef.current?.scrollIntoView({ behavior: "smooth" });
    }
  }, [session.chatMessages?.length, activeDrawer]);

  const handleSendMessage = async (e: FormEvent) => {
    e.preventDefault();
    if (!chatInput.trim()) return;
    const newMsg: ZoomChatMessage = {
      id: "msg-" + Date.now(),
    ...
    const updatedMessages = [...(session.chatMessages || []), newMsg];
    const updated = { ...session, chatMessages: updatedMessages };
    setSession(updated);
    try {
      const res = await CampusDatabase.updateZoomSession(String(session.id), { chatMessages: updatedMessages });
  ...
  const handleToggleHandRaise = async () => {
    ...
    const updatedAttendees = (session.activeAttendees || []).map((a) =>
      String(a.id) === String(currentUser.id) ? { ...a, isHandRaised: nextState } : a
    );
    let updatedMessages = session.chatMessages || [];
    if (nextState) {
      updatedMessages = [...updatedMessages, {
        ...
        message: `✋ ${currentUser.fullName} raised a hand to ask a question`,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
      }];
    }
    const updated = { ...session, activeAttendees: updatedAttendees, chatMessages: updatedMessages };
    setSession(updated);
    try {
  ...
  const handleLowerAllHands = async () => {
    const updatedAttendees = (session.activeAttendees || []).map((a) => ({ ...a, isHandRaised: false }));
    const updated = { ...session, activeAttendees: updatedAttendees };
    setSession(updated);
    try {
  ...
  const handleMuteAll = async () => {
    const updatedAttendees = (session.activeAttendees || []).map((a) =>
      a.role === "STUDENT" ? { ...a, isMuted: true } : a
    );
    const updated = { ...session, activeAttendees: updatedAttendees };
    setSession(updated);
    try {
  ...
  const handleEndSessionForEveryone = async () => {
    if (!confirm("End this Zoom lecture for all students and save attendance & recording?")) return;
    const updated = {
      ...session,
      ...
      lectureNotes: session.lectureNotes || "Official lecture concluded with recorded presentation and student Q&A."
    };
    setSession(updated);
    try {
      const res = await CampusDatabase.updateZoomSession(String(session.id), {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/85 backdrop-blur-md">
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-slate-900/90 border-b border-slate-800 shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <span className="font-semibold text-sm text-slate-100 truncate max-w-[280px] sm:max-w-md">
                    {session.courseCode}: {session.title}
                  </span>
                  <span className="hidden md:inline-flex items-center space-x-1 text-[11px] text-emerald-400 bg-emerald-950/50 border border-emerald-500/30 px-2 py-0.5 rounded-md">
            ...
            <div className="flex items-center space-x-2">
              <div className="flex items-center space-x-1.5 px-3 py-1 bg-slate-800/80 border border-slate-700/60 rounded-lg text-xs font-mono text-slate-300">
                ...
                <div className="flex-1 flex flex-col h-full space-y-3">
                  <div className="flex-1 relative rounded-xl bg-slate-950 border border-slate-800 flex flex-col overflow-hidden">
                    ...
                    <div className="absolute top-12 right-4 w-44 sm:w-56 h-32 sm:h-40 rounded-xl overflow-hidden shadow-2xl border-2 border-blue-500/80 bg-slate-900">
                      ...
                      <div className="px-2 py-1 bg-slate-950/90 flex items-center justify-between text-[11px]">
                        <span className="truncate text-slate-200 font-medium">{session.instructorName}</span>
                  ...
                  <div className="h-28 flex items-center space-x-2.5 overflow-x-auto pb-1 shrink-0">
                    <div className="w-36 h-24 rounded-xl bg-slate-950 border border-slate-700/80 p-1 flex flex-col justify-between shrink-0">
                      <div className="flex-1 flex items-center justify-center">
                        {isVideoOn ? (
                          <div className="relative">
                            <div className="w-10 h-10 rounded-full bg-blue-700 flex items-center justify-center font-bold text-xs text-white">
                              {currentUser.fullName.charAt(0)}
                            </div>
                            {isHandRaised && <span className="absolute -top-1 -right-2 text-sm">✋</span>}
                          </div>
                        ) : (
                      ...
                      <div className="flex items-center justify-between px-1.5 py-0.5 bg-slate-900/90 rounded text-[10px]">
                        <span className="truncate text-slate-300 font-medium">You</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 h-full overflow-y-auto">
                  <div className="aspect-video rounded-xl bg-slate-950 border-2 border-blue-500/60 p-2 flex flex-col justify-between">
                    <div className="flex-1 flex flex-col items-center justify-center">
                      <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white font-bold text-lg">
                        {session.instructorName.charAt(0)}
                      </div>
                      <span className="text-[10px] text-emerald-400 font-medium mt-1">Lecturing</span>
                    </div>
                    <div className="flex items-center justify-between px-2 py-1 bg-slate-900 rounded text-xs">
                      <span className="text-slate-200 font-semibold truncate">{session.instructorName} (Host)</span>
                      <Mic className="w-3.5 h-3.5 text-emerald-400" />
                    </div>
                  </div>
                  <div className="aspect-video rounded-xl bg-slate-950 border border-slate-800 p-2 flex flex-col justify-between">
                    <div className="flex-1 flex flex-col items-center justify-center">
                      <div className="w-14 h-14 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-lg">
                        {currentUser.fullName.charAt(0)}
                      </div>
                      {isHandRaised && <span className="text-base">✋</span>}
                    </div>
                    <div className="flex items-center justify-between px-2 py-1 bg-slate-900 rounded text-xs">
                      <span className="text-slate-200 font-semibold truncate">You</span>
                ...
                <div className="flex-1 flex flex-col bg-slate-950 border border-slate-800 rounded-xl p-4 overflow-y-auto">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Course: {session.courseCode}</p>
                    </div>
                  </div>
                  <div className="flex-1 mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-slate-900 rounded-xl border border-slate-800">
                      <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">Topic</h4>
                      <p className="text-xs text-slate-300 leading-relaxed mb-4">{session.topic}</p>
                      <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">Instructor Notes</h4>
                      <p className="text-xs text-slate-300 leading-relaxed bg-slate-950 p-3 rounded-lg border border-slate-800 font-mono">
                        {session.lectureNotes || "No supplementary notes uploaded yet."}
                      </p>
                    </div>
                    <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 flex flex-col">
                      <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">Live Code Demo</h4>
                      <div className="flex-1 bg-slate-950 p-3 rounded-lg border border-slate-800 text-xs font-mono text-emerald-300 overflow-x-auto">
                        <p className="text-slate-500"># Docker Compose</p>
                        <p className="text-blue-400">version: <span className="text-yellow-300">'3.8'</span></p>
                        <p className="text-blue-400">services:</p>
                        <p className="pl-4 text-purple-400">api-gateway:</p>
                        <p className="pl-8 text-slate-300">image: <span className="text-yellow-300">mau-cloud/gateway:v1.2</span></p>
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          return (
                            <div key={msg.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                              <div className="flex items-center space-x-1.5 mb-0.5">
                                <span className={`text-[11px] font-semibold ${msg.senderRole === "INSTRUCTOR" ? "text-emerald-400" : "text-blue-300"}`}>
                                  {msg.senderName}
                                </span>
                            ...
                            <div className="flex items-center space-x-2.5 truncate">
                              ...
                              <div className="truncate">
                                <span className="font-medium text-slate-200 block truncate">
                                  {att.name} {String(att.id) === String(currentUser.id) ? "(You)" : ""}
                                </span>
                                <span className="text-[10px] text-slate-400">Joined {att.joinedAt}</span>
                              </div>
                            </div>
                            <div className="flex items-center space-x-1.5 shrink-0">
                              {att.isHandRaised && <span className="text-xs">✋</span>}
                              {att.isMuted ? <MicOff className="w-3.5 h-3.5 text-slate-500" /> : <Mic className="w-3.5 h-3.5 text-emerald-400" />}
                            </div>
                          </div>
                        ))}
